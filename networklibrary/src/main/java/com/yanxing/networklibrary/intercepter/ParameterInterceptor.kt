...
class ParameterInterceptor() :Interceptor {
    ...
    override fun intercept(chain: okhttp3.Interceptor.Chain): Response {
        ...
        for (param in parameters){
            getParams.append(param).append("=").append(oldRequest.url().queryParameter(param)).append("  ")
        }
        ...
        val headerParamStr=if(TextUtils.isEmpty(headerParams)) "" else "  header参数"+headerParams
        LogUtil.d(TAG,newRequest.url().url().toString() + "  请求参数:" + getParams.toString() + postParams.toString() + headerParamStr
                    + "\n请求耗时：" + (a - b) + "ms，" + "请求结果\n" + content + "\n")

        val body = ResponseBody.create(if (newRequest.body() == null) null else newRequest.body()?.contentType(), content)
        //重新构造body
